...
int main()
{
    ...
    for (k = 0; k < a; k++)
    {
        ...
        for (j = k + 1; j < a; j++)
        {
            ...
            //tmpAO[k * a + j] = AO[k * a + j] / AO[k + k * a];
        }
        yO[k] = BO[k] / AO[k + k * a];
        AO[k + k * a] = 1;
//the second
#pragma omp parallel for num_threads(thread_count) default(none) private(i, j) shared(a, AO, BO, yO, k)
        //#pragma omp parallel for default(none) private(i, j) shared(a, AO, BO, yO, k)
        for (i = k + 1; i < a; i++)
        {
            for (j = k + 1; j < a; j++)
            {
    ...
    //#pragma omp parallel for num_threads(thread_count) private(k, i)
    for (k = a - 1; k >= 0; k--)
    {
        xO[k] = yO[k];
        //third
